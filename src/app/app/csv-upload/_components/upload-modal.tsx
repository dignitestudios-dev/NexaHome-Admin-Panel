  ...
  return (
    ...
    >
      <DialogPortal>
        ...
        <div className="fixed left-1/2 top-1/2 z-50 w-[min(520px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-2xl">
          <button
            type="button"
            onClick={handleCancel}
            disabled={uploadInvitations.isPending}
            className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 disabled:opacity-60"
            aria-label="Close"
          >
            <X className="h-5 w-5" />
          </button>
