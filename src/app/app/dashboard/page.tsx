            ...
            <CardContent className="p-0">
              <div className="p-4 flex items-center gap-5">
                <div className="bg-[#E6EEEE] p-5 rounded-2xl">
                  <item.icon className="w-8 h-8 text-[#004D4D]" strokeWidth={2.5} />
                </div>
                <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-2 rounded-[40px] border-none shadow-sm bg-white overflow-hidden h-[400px]">
          <CardContent className="p-0 flex h-full">
            <div className="flex-[3] p-10 flex flex-col justify-between">
              <h2 className="text-xl font-bold text-[#1A1A1A]">User Insights</h2>
                    ...
                    <div className="flex items-center justify-center py-6">
                      <div className="relative w-28 h-28">
                        <svg className="w-full h-full transform -rotate-90">
                          <circle cx="56" cy="56" r="48" stroke="#F1F4F4" strokeWidth="14" fill="none" />
                          <circle 
                            cx="56" cy="56" r="48" stroke="#005763" strokeWidth="14" fill="none" 
                            strokeDasharray="301.6" strokeDashoffset="96.5" strokeLinecap="round" 
                          />
                        </svg>
                      </div>
                    </div>
                    <div className="bg-[#F4F9F9] rounded-[24px] p-4 text-center">
                      <p className="text-[12px] font-semibold text-gray-600 mb-1 leading-tight">{chart.label}</p>
                      <p className="text-2xl font-black text-[#005763]">{chart.val}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex-1 bg-gradient-to-br from-[#005864] to-[#0A4D45] p-5 m-3 rounded-[35px] flex flex-col items-center justify-between">
              <div className="relative w-32 h-32 mt-6">
                <svg className="w-full h-full transform -rotate-90">
                  <circle cx="64" cy="64" r="52" stroke="white" strokeWidth="12" fill="none" className="opacity-20" />
                  <circle 
                    cx="64" cy="64" r="52" stroke="white" strokeWidth="12" fill="none" 
              ...
              <div className="w-full bg-white/10 rounded-[28px] py-5 flex flex-col items-center border border-white/5">
                <p className="text-xs font-medium text-white/80 mb-1">Average Rating</p>
                <div className="flex items-center gap-2">
                  <Star className="w-5 h-5 fill-[#FFB800] text-[#FFB800]" />
                  <span className="text-3xl font-black text-white italic">4.7</span>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="rounded-[40px] border-none shadow-sm bg-white p-8 h-[400px] flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-[#1A1A1A]">Growth Tracking</h2>
            ...
            {[
              ...
              <div key={i} className="flex-1 flex flex-col items-center gap-3 h-full justify-end z-10">
                <div className="flex gap-1.5 items-end h-full w-full justify-center">
                  <div className="w-3 bg-[#005864] rounded-full" style={{ height: `${d.u}%` }} />
                  <div className="w-3 bg-[#C8E015] rounded-full" style={{ height: `${d.e}%` }} />
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
             ...
             <div className="flex-1 flex items-end gap-[2px] pb-8 relative">
                {Array.from({ length: 60 }).map((_, i) => (
                  <div 
                    ...
                    style={{ height: `${Math.sin(i * 0.2) * 20 + 40 + (i % 8) * 4}%` }}
                  />
                ))}
             </div>
          </div>
        ...
        <Card className="rounded-[40px] border-none shadow-sm bg-white p-8">
          ...
          <div className="space-y-6">
            {[
              { name: "Window Cleaning", val: "10,000", width: "100%", color: "#004D4D" },
              { name: "Power Washing", val: "8,000", width: "80%", color: "#005864" },
              { name: "Soft Washing", val: "6,000", width: "65%", color: "#0FA3A3" },
              { name: "Window Cleaning", val: "4,000", width: "40%", color: "#0FA3A3" },
              { name: "Power Washing", val: "10,000", width: "100%", color: "#004D4D" },
              { name: "Soft Washing", val: "10,000", width: "100%", color: "#004D4D" },
            ].map((cat, i) => (
